...
| `number-conversion-other-semantics` <!-- compat:number-conversion-other-semantics=bounded --> | ⚠️ **Supported subset** | `SELECT '1,234.5' + 1 FROM dual` | Other implicit NUMBER conversions, precedence combinations, format models, and NLS-dependent parsing remain bounded. |
...
| `char-varchar-other-comparisons` <!-- compat:char-varchar-other-comparisons=bounded --> | ⚠️ **Supported subset** | `CAST('A' AS CHAR(3)) < CAST('B' AS VARCHAR2(3))` | Other CHAR/VARCHAR comparison/coercion shapes remain bounded to explicitly represented cases. |
...
| `date-timestamp-other-semantics` <!-- compat:date-timestamp-other-semantics=bounded --> | ⚠️ **Supported subset** | `TO_TIMESTAMP_TZ('2026-08-15 10:00 +09:00', 'YYYY-MM-DD HH24:MI TZH:TZM')` | Other DATE/TIMESTAMP coercion, precision, timezone, and environment-sensitive semantics require explicit case coverage. |
...
| `offset-fetch` <!-- compat:offset-fetch=same --> | ✅ **Supported** | `ORDER BY id OFFSET 20 ROWS FETCH NEXT 10 ROWS ONLY` | Ordered OFFSET/FETCH NEXT ROWS ONLY pagination represented by the contract. |
...
| `sequence-nextval-currval` <!-- compat:sequence-nextval-currval=same --> | ✅ **Supported** | `SELECT order_seq.NEXTVAL FROM dual` | Declared sequence NEXTVAL/CURRVAL behavior, including supported DML composition. |
...
| Area | Status | Example | Contract |
|---|---|---|---|
| `common-native-scalar-functions` <!-- compat:common-native-scalar-functions=same --> | ✅ **Supported** | `COALESCE(NULL, 7)`, `UPPER('OraFit')`, `ABS(-7)`, `TRIM('  Ab  ')` | Proven deterministic common scalar forms for `COALESCE`, `NULLIF`, `UPPER`, `LOWER`, `ABS`, `CEIL`, `FLOOR`, `MOD`, ANSI numeric `CAST`, `TRIM`, and `LENGTH` on represented numeric/non-empty ASCII inputs. NLS/collation and datatype edge cases are not implied. |
...
| `to-date` <!-- compat:to-date=same --> | ✅ **Supported** | `TO_DATE('2026-08-15', 'YYYY-MM-DD')` | Deterministic TO_DATE default/explicit formats, binds, punctuation flexibility, and represented Oracle errors. |
...
| `date-trunc-round` <!-- compat:date-trunc-round=same --> | ✅ **Supported** | `TRUNC(SYSDATE, 'MM')` | Oracle date TRUNC/ROUND day, month, year, quarter and represented half-unit boundaries. |
